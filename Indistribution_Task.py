...
def main():
    ...
    # Create dataloaders
    dataset, train_loader, val_loader, test_loader, test_indices = create_data_loaders(
        args, input_features, target_features, data)
    ...
    # Define experiment classes to run
    experiment_classes = [Experiment3]
    ...
    for experiment_class in experiment_classes:
        ...
        all_saved_models.sort(key=lambda x: x[0]) 
        ...
        result_file_path = os.path.join(experiment_dirs[experiment_class.__name__]['results'], 'convergence_and_early_stopping_new.txt')
        ...
        with torch.no_grad():
            for batch in test_loader:
                ...
                # Combine GPP_pred and ET_pred into a single array
        ...
        plt.grid(True, linestyle='--', alpha=0.7)
        plt.savefig(os.path.join(experiment_dirs[experiment_class.__name__]['plots'], 'averaged_prediction_GPP_new_scale_mar.png'), dpi=300)
        plt.close()
        ...
        plt.gca().set_ylim(top=1)
        plt.grid(True, linestyle='--', alpha=0.7)
        plt.savefig(os.path.join(experiment_dirs[experiment_class.__name__]['plots'], 'averaged_prediction_ET_new_scale_mar.png'), dpi=300)
        plt.close()
